<script lang="ts">
	...
	import { newWsStore } from '$lib/websockets/new-ws-store.js';
	...
	const clients = JSON.stringify({ type: 'chat', stream: 'combined:chat', strategy: 'streams' });
	const ws = newWsStore({ url: `${wsProtocol}//${host}/ws?clients=${clients}` });
