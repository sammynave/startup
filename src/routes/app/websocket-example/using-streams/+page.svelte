...
<Card.Root class="min-w-min md:w-[80%] lg:w-[60%] mx-auto border-none shadow-none">
	<Card.Header class="flex flex-row items-center justify-between">
		<div>
			<Card.Title>Streams strategy</Card.Title>
			<Card.Description>
				<div class="pt-2 pb-2">
					In this example, both the "presence" feature and the `chat` feature use Redis' streams
				</div>
				<form method="post" use:enhance>
					<Button size="sm" type="submit">Delete chat history</Button>
				</form>
			</Card.Description>
		</div>
		<div>
