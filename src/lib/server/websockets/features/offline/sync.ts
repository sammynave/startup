...
export class Sync {
	...
	*/
	...
	/*
			STATES
			- 1. connection
			- 2. server sends `connect` with server siteId
			- 3. client receives message and queries tracked_peers for server siteId and compares versions
				- A. Client is >= Server
					- 1. client queries for changes >= server's version
					- 2. client sends `update` with changes
						- 1. server receives updates
						- 2. server merges updates
						- 3. server sets tracked peers
						- 4. server sends `update` to all tracked_peers with version <= last change received
					- 3. client sets client version tracked_peers to last change sent
				- B. Client is <= Server
					- 1. client sends `request-for-update`
						- 1. server queries client's version in tracked_peers
						- 2. server queries for changes >= client's version
						- 3. server sends `update` with changes
